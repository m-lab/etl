...
# view definitions.
#
# ndt_all_legacysql_v3_1​ (standardSQL)
# Separate views for download and upload NDT tests (data ~ XX.XX.XXXX [date]):
# ​​​ndt_downloads_legacysql_v3_1 (standardSQL)
# ndt_uploads_legacysql_v3_1 (standardSQL)
...
VIEW=measurement-lab:internal.common_etl_legacysql_v3_1
echo $VIEW
bq rm -f $VIEW
bq mk \
--description=\
'ETL table projected into common schema, for union with PLX legacy data.
May 11, 2017 onward, to dovetail with plx, which includes up to May 10.' \
--view='#legacySQL
-- ETL table projected into common schema, for union with PLX legacy data.
-- May 11, 2017 onward, to dovetail with plx, which includes up to May 10.
SELECT
...
VIEW=measurement-lab:internal.ndt_exhaustive_legacysql_v3_1
echo $VIEW
bq rm -f $VIEW
bq mk \
--description=\
...
Note that at present, data from May 10 to July does NOT have geo annotations.' \
--view='#legacySQL
...
# Note that at present, data from May 10 to July does NOT have geo annotations.
SELECT * FROM
 [measurement-lab.internal.common_etl_legacysql_v3_1],
 [measurement-lab.legacy.ndt_plx_legacysql]' \
$VIEW

VIEW=measurement-lab:internal.ndt_all_legacysql_v3_1
echo $VIEW
bq rm -f $VIEW
bq mk \
...
--   blacklisted tests
select * from [measurement-lab.internal.ndt_exhaustive_legacysql_v3_1]
where 
-- not blacklisted
...
VIEW=measurement-lab:internal.ndt_sensible_legacysql_v3_1
echo $VIEW
bq rm -f $VIEW
bq mk \
...
select * from [measurement-lab.internal.ndt_all_legacysql_v3_1]
where 
...
VIEW=measurement-lab:internal.ndt_downloads_legacysql_v3_1
echo $VIEW
bq rm -f $VIEW
bq mk \
--description='All good quality download tests' \
--view='#legacySQL
select * from [measurement-lab.internal.ndt_sensible_legacysql_v3_1]
where
...
VIEW=measurement-lab:internal.ndt_uploads_legacysql_v3_1
echo $VIEW
bq rm -f $VIEW
bq mk \
...
VIEW=measurement-lab:public.ndt_all_legacysql_v3_1
echo $VIEW
bq rm -f $VIEW
bq mk \
--description='View across the all NDT data except EB and blacklisted' \
...
VIEW=measurement-lab:public.ndt_downloads_legacysql_v3_1
echo $VIEW
bq rm -f $VIEW
bq mk \
--description='All good quality download tests' \
--view='#legacySQL
select * from [measurement-lab.internal.ndt_downloads_legacysql_v3_1]' \
$VIEW

VIEW=measurement-lab:public.ndt_uploads_legacysql_v3_1
echo $VIEW
bq rm -f $VIEW
bq mk \
--description='All good quality upload tests' \
--view='#legacySQL
#legacySQL
select * from [measurement-lab.internal.ndt_uploads_legacysql_v3_1]' \
$VIEW
