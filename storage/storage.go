...
func (src *GCSSource) nextHeader(trial int) (*tar.Header, bool, error) {
	...
	if esrc != nil {
		if esrc == io.EOF {
			return nil, false, esrc
		} else if strings.Contains(esrc.Error(), "unexpected EOF") {
...
func (src *GCSSource) nextData(h *tar.Header, trial int) ([]byte, bool, error) {
	...
	if strings.HasSuffix(strings.ToLower(h.Name), "gz") {
		...
		zipReader, esrc = gzip.NewReader(src)
		if esrc != nil {
			if esrc == io.EOF {
				return nil, false, esrc
			}
			...
			return nil, true, esrc
		}
		defer zipReader.Close()
		phase = "read zip"
		data, esrc = ioutil.ReadAll(zipReader)
	} else {
		phase = "read"
		data, esrc = ioutil.ReadAll(src)
	}
	if esrc != nil {
		// These errors seem to be recoverable, at least with zip files.
		if strings.Contains(esrc.Error(), "stream error") {
...
func (src *GCSSource) NextTest(maxSize int64) (string, []byte, error) {
	...
	for {
		trial++
		var retry bool
		h, retry, esrc = src.nextHeader(trial)
		if esrc == nil {
			break
		}
		if !retry || trial >= 10 {
	...
	for {
		trial++
		var retry bool
		data, retry, esrc = src.nextData(h, trial)
		if esrc == nil {
			break
		}
		if !retry || trial >= 10 {
...
func NewTestSource(client *storage.Client, uri string, label string) (etl.TestSource, error) {
	...
	rdr, cancel, esrc := getReader(client, bucket, fn, 300*time.Minute)
	if esrc != nil {
		cancel()
		log.Println(esrc)
		return nil, esrc
...
func GetStorageClient(writeAccess bool) (*storage.Client, error) {
	...
	// the cancel.
	client, esrc := storage.NewClient(context.Background(), option.WithScopes(scope))
	if esrc != nil {
		return nil, esrc
	}
	return client, nil
...
func getReader(client *storage.Client, bucket string, fn string, timeout time.Duration) (io.ReadCloser, func(), error) {
	...
	rdr, esrc := obj.NewReader(ctx)
	return rdr, cancel, esrc
}
