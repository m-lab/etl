...
func CreateOrUpdateNDTRow(project string, dataset string, table string) error {
	row := schema.NDTRow{}
	schema, err := row.Schema()
	rtx.Must(err, "NDTRow.Schema")
	return CreateOrUpdate(schema, project, dataset, table)
}
...
func main() {
	...
	case "all": // Do everything
		...
		if err := CreateOrUpdateNDTRow(project, "base_tables", "ndt5"); err != nil {
			errCount++
		}
		if err := CreateOrUpdateNDTRow(project, "batch", "ndt5"); err != nil {
			errCount++
		}
