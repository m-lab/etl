...
func TestScamper1Parser_ParseAndInsert(t *testing.T) {
	tests := []struct {
		file    string
		wantErr bool
		wantRow schema.Scamper1Row
	}{
		{"invalid-metadata.jsonl", true, schema.Scamper1Row{}},
		{"invalid-cycle-start.jsonl", true, schema.Scamper1Row{}},
		{"invalid-tracelb.jsonl", true, schema.Scamper1Row{}},
		{"invalid-cycle-stop.jsonl", true, schema.Scamper1Row{}},
		{"valid.jsonl", false, expectedScamper1Row()},
	}
	...
	for _, tt := range tests {
		...
		err = n.ParseAndInsert(meta, tt.file, data)
		if tt.wantErr == (err == nil) {
			t.Errorf("Scamper1Parser.ParseAndInsert() error = %v, wantErr %v", err, tt.wantErr)
		}
		...
		if n.Accepted() == 1 {
			...
			tt.wantRow.Parser.Time = row.Parser.Time
			if diff := deep.Equal(row, &tt.wantRow); diff != nil {
...
func expectedScamper1Row() schema.Scamper1Row {
	...
	bqScamperNode1 := schema.BQScamperNode{
		UUID:  "20190825_ndt-plh7v_2001:550:1b01:1::1",
	...
	bqScamperNode2 := schema.BQScamperNode{
		UUID:  "20190825_ndt-plh7v_2001:4888:3f:6092:3a2:26:0:1",
